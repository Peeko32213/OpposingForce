...
public class CustomCollisionsMoveControl extends MoveControl {

    public CustomCollisionsMoveControl(Mob mob) {
        super(mob);
    }
    ...
    public void tick() {
        if (this.operation == MoveControl.Operation.STRAFE) {
            ...
            if (!(mob instanceof CustomCollisions && ((CustomCollisions) mob).canPassThrough(blockpos, blockstate, voxelshape))) {
    ...
    private boolean isWalkable(float v, float v1) {
        PathNavigation pathnavigator = this.mob.getNavigation();
        if (pathnavigator != null) {
            NodeEvaluator nodeprocessor = pathnavigator.getNodeEvaluator();
            return nodeprocessor == null || nodeprocessor.getBlockPathType(this.mob.level(), Mth.floor(this.mob.getX() + (double) v), Mth.floor(this.mob.getY()), Mth.floor(this.mob.getZ() + (double) v1)) == BlockPathTypes.WALKABLE;
        }
        return true;
    }
}