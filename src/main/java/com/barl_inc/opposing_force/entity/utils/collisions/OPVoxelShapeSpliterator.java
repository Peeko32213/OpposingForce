...
public class OPVoxelShapeSpliterator extends Spliterators.AbstractSpliterator<VoxelShape> {
    ...
    public OPVoxelShapeSpliterator(CollisionGetter reader, @Nullable Entity entity, AABB aabb) {
        this(reader, entity, aabb, (p_241459_0_, p_241459_1_) -> {
            return true;
        });
    }

    public OPVoxelShapeSpliterator(CollisionGetter reader, @Nullable Entity entity, AABB aabb, BiPredicate<BlockState, BlockPos> statePositionPredicate) {
    ...
    boolean collisionCheck(Consumer<? super VoxelShape> p_234878_1_) {
        while (true) {
            if (this.cubeCoordinateIterator.advance()) {
                ...
                if (entity instanceof CustomCollisions && ((CustomCollisions) entity).canPassThrough(mutablePos, blockstate, voxelshape)) {
                    continue;
                }
