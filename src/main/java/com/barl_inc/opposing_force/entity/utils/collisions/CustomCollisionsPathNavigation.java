package com.barl_inc.opposing_force.entity.utils.collisions;
...
public class CustomCollisionsPathNavigation extends GroundPathNavigation {

    public CustomCollisionsPathNavigation(Mob mob, Level world) {
        super(mob, world);
    }
    ...
    private boolean isPositionClear(int x, int y, int z, int sizeX, int sizeY, int sizeZ, Vec3 p_179692_7_, double p_179692_8_, double p_179692_10_) {
        for (BlockPos blockpos : BlockPos.betweenClosed(new BlockPos(x, y, z), new BlockPos(x + sizeX - 1, y + sizeY - 1, z + sizeZ - 1))) {
            ...
            if (!(d0 * p_179692_8_ + d1 * p_179692_10_ < 0.0D) && !this.level.getBlockState(blockpos).isPathfindable(this.level, blockpos, PathComputationType.LAND) || ((CustomCollisions) mob).canPassThrough(blockpos, level.getBlockState(blockpos), null)) {
                return false;
            }
        }
            ...
            for (int k = i; k < i + sizeX; ++k) {
                for (int l = j; l < j + sizeZ; ++l) {
                    ...
                    if (!(d0 * p_179683_8_ + d1 * p_179683_10_ < 0.0D)) {
                        ...
                        mutable.set(k, y - 1, l);
                        if (!this.hasValidPathType(pathnodetype) || ((CustomCollisions) mob).canPassThrough(mutable, level.getBlockState(mutable), null)) {
                            return false;
                        }
