...
public class DicerVisorLayer extends RenderLayer<Dicer, DicerModel> {
    ...
    public void render(@NotNull PoseStack poseStack, MultiBufferSource multiBufferSource, int i, Dicer entity, float f, float g, float h, float j, float k, float l) {
        VertexConsumer vertexConsumer = multiBufferSource.getBuffer(entity.getPose() == OPPoses.LASERING.get() ? (entity.isElite() ? ARCH_VISOR_LASERING : VISOR_LASERING) : (entity.isElite() ? ARCH_VISOR : VISOR));
        if (entity.getName().getString().contains("gigan")) {
            vertexConsumer = multiBufferSource.getBuffer(entity.getPose() == OPPoses.LASERING.get() ? GIGAN_VISOR_LASERING : GIGAN_VISOR);
        }
