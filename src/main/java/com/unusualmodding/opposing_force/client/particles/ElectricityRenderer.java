...
public class ElectricityRenderer {
    ...
    public void update(Object owner, ElectricBoltData newBoltData, float partialTicks) {
    ...
    public class Timestamp {
        ...
        public Timestamp subtract(Timestamp other) {
            ...
            return ElectricityRenderer.this.new Timestamp(newTicks, newPartial);
        }
    ...
    public class BoltOwnerData {
        ...
        private ElectricBoltData lastBolt;
        private Timestamp lastBoltTimestamp = ElectricityRenderer.this.new Timestamp();
        private Timestamp lastUpdateTimestamp = ElectricityRenderer.this.new Timestamp();
        ...
        private void addBolt(BoltInstance instance, Timestamp timestamp) {
            ...
            this.lastBoltDelay = (double)instance.bolt.getSpawnFunction().getSpawnDelay(ElectricityRenderer.this.random);
            this.lastBoltTimestamp = timestamp;
        }
    }

    public class BoltInstance {
        private final ElectricBoltData bolt;
        private final List<ElectricBoltData.BoltQuads> renderQuads;
        private Timestamp createdTimestamp;

        public BoltInstance(ElectricBoltData bolt, Timestamp timestamp) {
            this.bolt = bolt;
        ...
        public void render(Matrix4f matrix, VertexConsumer buffer, Timestamp timestamp) {
            ...
            for(int i = (Integer)bounds.getLeft(); i < (Integer)bounds.getRight(); ++i) {
                ((ElectricBoltData.BoltQuads)this.renderQuads.get(i)).getVecs().forEach((v) -> {
