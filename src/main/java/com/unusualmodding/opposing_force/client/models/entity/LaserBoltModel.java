...
public class LaserBoltModel extends HierarchicalModel<LaserBolt> {
	...
	public static LayerDefinition createProjectileLayer() {
        ...
        root.addOrReplaceChild("outer", CubeListBuilder.create()
                .texOffs(0, 0).addBox(-1.5F, -3.0F, -4.5F, 3.0F, 3.0F, 9.0F, new CubeDeformation(0.0F)), PartPose.ZERO);

        root.addOrReplaceChild("inner", CubeListBuilder.create()
                .texOffs(0, 12).addBox(-1.0F, -2.5F, -4.0F, 2.0F, 2.0F, 8.0F, new CubeDeformation(0.0F)), PartPose.ZERO);

        return LayerDefinition.create(meshdefinition, 32, 32);
	}

	@Override
	public void setupAnim(@NotNull LaserBolt entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
	}
