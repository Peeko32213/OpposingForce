...
public class OPRecipeProvider extends RecipeProvider {
    ...
    protected void buildRecipes(@NotNull Consumer<FinishedRecipe> consumer) {
        ...
        ShapedRecipeBuilder.shaped(COMBAT, TESLA_CANNON.get()).define('#', TESLA_INGOT.get()).define('X', DEEP_SILK.get()).define('Y', Tags.Items.RODS_WOODEN).define('Z', Blocks.TRIPWIRE_HOOK).define('A', ELECTRIC_CHARGE.get()).pattern("#A#").pattern("XZX").pattern(" Y ").unlockedBy("has_tesla_ingot", has(TESLA_INGOT.get())).save(consumer);
        ShapedRecipeBuilder.shaped(MISC, TESLA_INGOT.get(), 2).define('A', ELECTRIC_CHARGE.get()).define('B', Tags.Items.INGOTS_GOLD).define('C', Tags.Items.GEMS_DIAMOND).pattern(" A ").pattern("BCB").pattern(" A ").unlockedBy("has_electric_charge", has(ELECTRIC_CHARGE.get())).save(consumer);
        ShapedRecipeBuilder.shaped(COMBAT, SPARK_BLADE.get()).define('B', Tags.Items.INGOTS_GOLD).define('A', TESLA_INGOT.get()).pattern(" A ").pattern(" A ").pattern(" B ").unlockedBy("has_tesla_ingot", has(TESLA_INGOT.get())).save(consumer);
