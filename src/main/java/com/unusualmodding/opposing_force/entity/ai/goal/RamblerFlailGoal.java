...
public class RamblerFlailGoal extends AttackGoal {
    ...
    public void tick() {
        ...
        if (target != null) {
            ...
            if (this.rambler.isFlailing()) {
                ...
                if (this.timer > 20 && this.timer < 200) {
                ...
                if (this.rambler.flailCooldown == 0) {
                    this.rambler.getNavigation().moveTo(target, 1.5D);
    ...
    private void hurtNearbyEntities() {
        List<LivingEntity> nearbyEntities = rambler.level().getNearbyEntities(LivingEntity.class, TargetingConditions.forCombat(), rambler, rambler.getBoundingBox().inflate(1.1));
