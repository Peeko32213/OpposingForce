...
public class EntityTrembler extends AbstractMonster implements GeoAnimatable, GeoEntity {
    private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);

    private static final EntityDataAccessor<Integer> CHARGE_COOLDOWN_TICKS = SynchedEntityData.defineId(EntityTrembler.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Boolean> HAS_TARGET = SynchedEntityData.defineId(EntityTrembler.class, EntityDataSerializers.BOOLEAN);
    ...
    public EntityTrembler(EntityType<? extends Monster> pEntityType, Level pLevel) {
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return Mob.createMobAttributes()
                ...
                .add(Attributes.KNOCKBACK_RESISTANCE, 100.00D);
    }
    ...
    public static <T extends Mob> boolean canFirstTierSpawn(EntityType<EntityTrembler> entityType, ServerLevelAccessor iServerWorld, MobSpawnType reason, BlockPos pos, RandomSource random) {
        return reason == MobSpawnType.SPAWNER || !iServerWorld.canSeeSky(pos) && pos.getY() <= 30 && checkUndergroundMonsterSpawnRules(entityType, iServerWorld, reason, pos, random);
    }
    ...
    protected <E extends EntityTrembler> PlayState Controller(final software.bernie.geckolib.core.animation.AnimationState<E> event) {
        if (this.stunnedTick > 0) {
    ...
    static class TremblePrepareChargeGoal extends Goal {
        protected final EntityTrembler tremble;

        public TremblePrepareChargeGoal(EntityTrembler tremble) {
    ...
    static class TrembleChargeGoal extends Goal {
        protected final EntityTrembler mob;
        private final double speedModifier;
        private Path path;
        private Vec3 chargeDirection;

        public TrembleChargeGoal(EntityTrembler pathfinderMob, double speedModifier) {
        ...
        protected void tryToHurt() {
            ...
            if (!nearbyEntities.isEmpty()) {
                ...
                if (!(livingEntity instanceof EntityTrembler)) {
    ...
    static class TrembleMeleeAttackGoal extends MeleeAttackGoal {
        ...
        protected void checkAndPerformAttack(LivingEntity enemy, double distToEnemySqr) {
            ...
            if (distToEnemySqr <= d0 && this.getTicksUntilNextAttack() <= 0) {
                ...
                ((EntityTrembler) this.mob).attack(enemy);
            }
        }
